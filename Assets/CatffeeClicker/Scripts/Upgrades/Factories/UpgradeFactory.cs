...
public class UpgradeFactory
{
    ...
    public UpgradesStorage Create(string name, string description, int price, int value, UpgradeType type, Sprite icon)
    {
        if (!_cache.ContainsKey(name))
        {
            var upgrade = new UpgradesStorage(name, description, price, value, type, icon);
            _cache[name] = upgrade;
