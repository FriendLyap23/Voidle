...
public class MainButtonAnimations : MonoBehaviour
{
    private MainButton _moneyClickButton;
    ...
    {
        _moneyClickButton = GetComponent<MainButton>();
    ...
    private void TapAnimation() 
    {
        ...
        _tapAnimation = sequence.Append(_moneyClickButton.transform.DOScale(1.5f, 0.5f).From(2).SetEase(Ease.OutSine))
            .Join(_moneyClickButton.transform.DOScale(2, 0.5f).From(1.5f).SetEase(Ease.OutSine));
    }
